...
public class Controller {
	...
	private int parseInt(String value) throws InvalidInputException{
		try{
	...
	private void addHouse(){
		...
		try{
			int squareFootageInt = parseInt(squareFootage);
			int costInt = parseInt(cost);
			if(type.equals("t")){
				System.out.println("Please enter timber type.");
				String timberType = scan.next();
				model.addTimberHouse(name, squareFootageInt, costInt, roomList, timberType);
			}else{
				System.out.println("Please enter the maximum number of floors.");
				String maxFloors = scan.next();
				int maxFloorsInt = parseInt(maxFloors);
				model.addMasonryHouse(name, squareFootageInt, costInt, roomList, maxFloorsInt);			
			}
		}catch(InvalidInputException e){
			System.out.println("You entered " + e.getInput() + " when I was expecting a number!");
			e.printStackTrace();
			System.exit(0);
		}
